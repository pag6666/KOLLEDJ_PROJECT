...
def get_price_from_globus(url):
    ...
    # Извлекаем цену
    price_tag = soup.find('span', class_='c-prices__value js-prices_pdv_ГЛОБУС Розничная')  # Замените на актуальный класс для цены
    if price_tag:
...
def save_to_csv(data:list[list], filename):
